...
static ssize_t lib_eval(struct cx_rmacro_eval *eval,
			    struct cx_bin *bin,
			    size_t tok_idx,
			    struct cx *cx) {
...
static ssize_t use_eval(struct cx_rmacro_eval *eval,
			struct cx_bin *bin,
			size_t tok_idx,
			struct cx *cx) {
...
cx_lib(cx_init_meta, "cx/meta") {
  ...
  cx_add_rmacro(lib, "lib:", lib_parse);
  cx_add_rmacro(lib, "use:", use_parse);
