...
* add cx_type.meta CX_TYPE_TRAIT/ARG/TYPE/REC
** add cx_type_init arg
** replaces cx_type.trait
* add Stack type args
...
** always forward id
** remove lookup flag
* add support for rec type args
** implement type_new method for rec type
** add cx_arg_ref with vec indices
** generate on demand in *lobby from cx_get_type if not found
*** with type args if provided
*** search *lobby directly if name starts with 'ArgN'
** replace field types in type_init method if CX_TYPE_ARG
