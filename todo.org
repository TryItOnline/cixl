* add qdb.cx
** QDBTable with key, rec-keys & key-recs
*** new-table takes vect of key callables
*** rec-keys uses recs as keys
*** key-recs vectors
** implement Rec Cmp to compare pointers
** implement Vec Cmp to compare values
** add built-in Table type
*** use instead of files in bookings
* change _ to func
** add stack lib
*** move |, %, %%, ~ & _
