...
* thread emit loop using computed goto
** store array of labels indexed by pc
** remember pc on label enter
*** goto on exit if changed
** check stop inside every case
** remove outer loop
** remove op.emit_break
** test stopped in each case
** attempt inlining labels in emit
