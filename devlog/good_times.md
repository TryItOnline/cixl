...
### The Forth/Lisp Way
One of the basic tenets of Forth/Lisp is to provide fewer concepts and use them for everything, almost to a fault. Doing so leads to more composable and powerful APIs with less duplication. Most other languages unfortunately didn't get the memo, time libraries even less so; the people responsible for Python's seem to have read the memo upside down and added as many distinct freaking [concepts](https://docs.python.org/3.5/library/datetime.html) as they could possibly think of. Needless to say, I decided that [cixl](https://github.com/basic-gongfu/cixl) would be better of following Forth's and Lisp's lead.
